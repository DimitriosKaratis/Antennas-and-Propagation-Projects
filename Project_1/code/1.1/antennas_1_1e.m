...
for i = 1:length(theta_m)
    for j = 1:length(phi_m)
        ...
        E0_abs = abs(h0 * I0 * k * l * sin(theta) / (4*pi*r)); % Electric field constant E0
        ...
        Theta_z = calculate_HPBW_for_broadside(lambda, Nz, d);  % Broadside in z(x')
        Theta_x = 105.4 * sqrt(lambda / (Nx* d));               % End-fire in x(y') 
